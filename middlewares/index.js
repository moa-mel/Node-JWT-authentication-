...
//middleware fuction to check if the incoming request in authenticated:
exports.checkAuth = (req, res, next) =>{
    //get the token stored in the custom header called 'x-auth-token'
    const token = req.get("x-auth-token");
//send error message if no token missing:
    if(!token){
...
//token can be expired or invalid. Send appropraite errors in each case:
